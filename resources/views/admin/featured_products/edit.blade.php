...
@section('page-title', 'Edit Featured Product')
...
<div class="max-w-2xl">
    ...
    <form action="{{ route('admin.featured-products.update', $featuredProduct->id) }}" method="POST" enctype="multipart/form-data" class="bg-[#151515] border border-[#2a2a2a] rounded-lg p-8 shadow-lg">
        @csrf
        @method('PUT')

        <div class="mb-6">
            <label for="title" class="block text-sm font-semibold text-[#cccccc] mb-2 uppercase tracking-wide">Product Title *</label>
            <input type="text" id="title" name="title" value="{{ old('title', $featuredProduct->title) }}" 
        ...
        <div class="mb-8">
            ...
            <textarea id="description" name="description" rows="5" 
                      ...
                      required placeholder="Enter detailed product description">{{ old('description', $featuredProduct->description) }}</textarea>
        </div>

        <div class="flex gap-4">
            <button type="submit" class="px-8 py-3.5 bg-gradient-to-r from-[#d4af37] to-[#ffd700] text-[#1a1a1a] border-none rounded-md font-bold text-base cursor-pointer transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_8px_20_rgba(255,215,0,0.2)]">Update Product</button>
